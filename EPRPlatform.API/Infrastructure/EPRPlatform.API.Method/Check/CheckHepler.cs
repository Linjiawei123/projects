...
namespace EPRPlatform.API.Method.Check
{
    /// <summary>
    /// 验证
    /// </summary>
    public class ValidHepler
    {
        #region IsEmail(是否邮箱)
        ...
        #region IsMainModels(是否主域名)
        /// <summary>
        /// 是否主域名或者www开头的域名
        /// </summary>
        /// <param name="value">url地址</param>
        /// <returns></returns>
        public static bool IsMainModels(string value)
        {
            if (string.IsNullOrWhiteSpace(value))
            {
                return false;
        ...
        /// <param name="IPRange">ip段数组用“~”分隔</param>
        /// <returns></returns>
        public static bool EffectiveIP(string ip, string[] IPRange)
        {
            if (ip == "::1")
                return true;
            long loginIP = IPToLongNum(ip);
            foreach (var s in IPRange)
            {
                string[] IpR = s.Split('~');
                long startIP = IPToLongNum(IpR[0].Trim());
                long endIP = IPToLongNum(IpR[1].Trim());
                if (loginIP >= startIP && loginIP <= endIP)
                    return true;
            }
            return false;
        }
