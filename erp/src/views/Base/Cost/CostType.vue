    ...
    <div class="box">
                ...
                <el-table-column label="分类编号" style="width: 30%;">
                    <template #default="scope">
                        <div style="display: flex; align-items: center">
                            <span>{{ scope.row.costTypeCode }}</span>
                        </div>
                    </template>
                </el-table-column>
                ...
                <el-table-column fixed="right" label="操作" style="width: 40%;">
                    <template #default="scope">
                        <el-button link type="warning" size="small" @click="clickEditHandle(scope.row)"
                            v-if="this.$isHasRgihts('Role_Update') && scope.row.isHasCost === false"><el-icon>
                                <Edit />
                            </el-icon></el-button>
                        <el-popconfirm title="确定要删除该分类？" icon="WarningFilled" icon-color="#ef4136"
                            @confirm="clickDeleteHandle(scope.row.id)"
                            v-if="this.$isHasRgihts('Role_Delete') && scope.row.isHasCost === false">
                            <template #reference>
        ...
        <el-drawer v-model="drawerVisible" :with-header="false" :show-close="false" size="30%">
            ...
            <el-form ref="form" :model="form" :rules="rules" :label-width="100">
                <el-form-item label="分类编号：" prop="costTypeCode">
                    <el-input v-model="form.costTypeCode" />
                </el-form-item>
                <el-form-item label="分类名称：" prop="costTypeName">
                    <el-input v-model="form.costTypeName" />
                </el-form-item>
            </el-form>
...
export default {
    ...
    data() {
        return {
            ...
            operate: 0,
            rules: {
                costTypeCode: [{ required: true, message: '请输入分类编号', trigger: 'blur' }],
                costTypeName: [{ required: true, message: '请输入分类名称', trigger: 'blur' }]
            }
        }
    },
    mounted() {
    ...
    methods: {
        getList() {
            axios.get(path.baseUrl + path.CostType)
                .then(res => {
        ...
        confirmAdd() {
            this.$refs['form'].validate(valid => {
                if (valid) {
                    axios.post(path.baseUrl + path.CostType, this.form).then(res => {
                        if (res.data.status) {
        ...
        confirmEdit() {
            this.$refs['form'].validate(valid => {
                if (valid) {
                    axios.put(path.baseUrl + path.CostType, this.form).then(res => {
                        if (res.data.status) {
                            this.drawerVisible = false;
                            this.getList();
                        }
        ...
        clickDeleteHandle(Id) {
            axios.delete(path.baseUrl + path.CostType + '/' + Id)
                .then(res => {
                    this.getList();
                }).catch(err => {
                    console.log(err);
