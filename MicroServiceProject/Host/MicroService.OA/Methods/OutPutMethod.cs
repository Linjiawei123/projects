...
namespace MicroService.OA
{
    ...
    public static class OutPutMethod<T>
    {
        ...
        /// <returns></returns>
        public static OutPutModel<T> Failure(string message = "服务器错误", T data = default, int statusCode = (int)HttpStatusCode.InternalServerError)
        {
            BaseHttpContext.Current.Response.StatusCode = statusCode;

            OutPutModel<T> outPutModel = new()
            {
                Status = false,
                ErrType = 1,
                Message = string.IsNullOrWhiteSpace(message) ? "服务器错误" : message.Trim(),
                Rights = new List<string>(),
                Data = data
            };
